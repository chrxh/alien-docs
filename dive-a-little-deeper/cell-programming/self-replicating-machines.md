...
{% hint style="info" %}
It should be noted that the scanner cell stores the internal state of the scanned cell (specialization, memory, etc. ), angle and distance at the same location in the token memory where the constructor reads them out. Therefore we do not have to copy this information.
{% endhint %}
